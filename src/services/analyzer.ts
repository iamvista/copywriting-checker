...
function analyzeFAB(text: string): DimensionScore {
  ...
  if (hasFeature) {
    ...
    feedback.push(`✓ ${fabRules.rules.feature.scoring.message.success}`)
  } else {
    feedback.push(`⚠️ ${fabRules.rules.feature.scoring.message.warning}`)
  }
  ...
  // 根據 FAB 完整度給予額外建議
  if (score < 60) {
    suggestions.push('💡 Vista 提醒：不要只停留在 F（特性）層面，一定要說到 B（效益）')
  }
...
function analyzeConsumerInsight(text: string): DimensionScore {
  ...
  if (hasEmotion) {
    ...
    feedback.push('✗ 缺少情緒連結。')
  }

  // 檢查同理心表達
  const empathyWords = ['我懂', '也曾經', '相信您', '了解', '知道', '明白']
  const hasEmpathy = containsKeywords(text, empathyWords)

  if (hasEmpathy) {
    score += 20
    feedback.push('✓ 有同理心表達。')
  } else {
    feedback.push('✗ 缺少同理心表達。')
...
function analyzeReadability(text: string): DimensionScore {
  ...
  if (hasGoodParagraphs) {
    score += 25
    feedback.push('✓ 段落結構適中。')
  } else {
    score += 5
    feedback.push('✗ 段落過長，建議適當分段。')
  ...
  if (hasWhitespace) {
    score += 15
    feedback.push('✓ 有適當留白，閱讀舒適。')
  } else {
    score += 5
    feedback.push('✗ 內容過於密集。')
...
function generateSummary(
  ...
  const dimensionScores = [
    { name: 'FAB 法則', key: 'fab', score: dimensions.fab.score },
    { name: '標題吸引力', key: 'titleAppeal', score: dimensions.titleAppeal.score },
    { name: '消費者洞察', key: 'consumerInsight', score: dimensions.consumerInsight.score },
    { name: '行動呼籲', key: 'callToAction', score: dimensions.callToAction.score },
    { name: '可讀性', key: 'readability', score: dimensions.readability.score },
    { name: '價值主張', key: 'valueProposition', score: dimensions.valueProposition.score },
  ]
  ...
  // 第一段：開場評語和總體印象
  if (grade === 'excellent') {
    overallAssessment = `經過全面分析，您的文案整體表現優異，獲得 ${totalScore} 分的高分！這份文案在多個面向都展現了專業水準，顯示您對於文案寫作的核心原則有相當程度的掌握。`
  } else if (grade === 'good') {
    overallAssessment = `整體來看，您的文案已經具備紮實的基礎，總分達到 ${totalScore} 分。這個分數顯示您已經理解文案的基本架構，並且在某些維度上有不錯的表現。`
  } else if (grade === 'needsImprovement') {
    overallAssessment = `經過仔細分析，您的文案目前獲得 ${totalScore} 分。雖然還有進步空間，但這其實是個很好的起點！這個分數代表您的文案已經具備基本架構，只要針對幾個關鍵環節進行優化，就能看到顯著的提升。文案寫作是可以快速進步的技能，您已經踏出重要的第一步。`
  } else {
    overallAssessment = `感謝您使用文案健檢工具！這份文案目前得到 ${totalScore} 分，我知道這個分數可能不如預期，但請別氣餒。文案寫作確實需要一些技巧和方法，但好消息是，這些都是可以透過學習和練習快速掌握的。許多優秀的文案創作者也是從零開始，一步步累積經驗。讓我們一起來看看如何優化這份文案，您會發現改善的過程其實很有成就感！`
  }

  // 第二段：詳細的強項分析
  overallAssessment += '\n\n'
  if (strongest[0].score >= 70) {
    overallAssessment += `從六大維度來看，您在「${strongest[0].name}」方面表現最為突出，獲得 ${strongest[0].score} 分。`
    if (strongest[1].score >= 70) {
      overallAssessment += `「${strongest[1].name}」也達到 ${strongest[1].score} 分的良好水準。`
    }
    overallAssessment += '這些優勢是很好的基礎，顯示您已經掌握了這些領域的重點。'

    // 根據強項給予具體正面回饋
    if (strongest[0].key === 'fab' && strongest[0].score >= 70) {
      overallAssessment += '特別是 FAB 法則的運用，這是文案最核心的架構，能做到這一點非常不容易。'
    } else if (strongest[0].key === 'titleAppeal' && strongest[0].score >= 70) {
      overallAssessment += '尤其是標題的吸引力，好的標題等於成功了一半，這會大幅提升您的點擊率。'
    }
  } else {
    overallAssessment += `從分析結果來看，六大維度目前的表現都還有很大的提升空間。這代表我們需要從基礎開始，系統性地強化每個環節。`
  }
  ...
  if (criticalWeaknesses.length > 0) {
    overallAssessment += `另一方面，我特別注意到「${criticalWeaknesses[0].name}」目前只有 ${criticalWeaknesses[0].score} 分，`
    if (criticalWeaknesses.length > 1) {
      overallAssessment += `而「${criticalWeaknesses[1].name}」也只有 ${criticalWeaknesses[1].score} 分，`
    }
    overallAssessment += '這是目前最需要優先改善的部分。'

    // 針對 FAB 弱項給予特別提醒
    if (dimensions.fab.score < 50) {
      overallAssessment += '特別要提醒的是，FAB 法則（特性、優勢、效益）是文案的核心架構，這個部分的弱化會直接影響文案的說服力。很多文案只停留在「Feature（特性）」的層面，卻忘了告訴讀者「對我有什麼好處（Benefit）」，這是最常見也最致命的錯誤。'
    }
  } else if (weakest[0].score < 60) {
    overallAssessment += `相對而言，「${weakest[0].name}」得分 ${weakest[0].score} 分，是目前比較需要加強的環節。`
    if (weakest[1].score < 60) {
      overallAssessment += `「${weakest[1].name}」（${weakest[1].score} 分）也值得關注。`
    }
  }
  ...
  overallAssessment += '\n\n'
  if (grade === 'excellent') {
    overallAssessment += '以您目前的水準，接下來可以朝向「精益求精」的方向努力。建議您針對不同的目標受眾進行 A/B 測試，透過數據回饋來微調文案的每個細節。同時，也可以開始研究更進階的文案技巧，例如故事行銷、情感連結等手法。'
  } else if (grade === 'good') {
    overallAssessment += '您的文案已經有 60 分以上的水準，只要針對幾個關鍵點進行調整，就能讓效果更上一層樓。建議您特別關注下方的「優先改善建議」，這些都是經過分析後最值得投入時間優化的部分。每改善一個弱點，都會為您的文案加分。'
  } else if (grade === 'needsImprovement') {
    overallAssessment += '建議您先從下方列出的「優先改善建議」開始，一步步優化文案品質。不用急著一次改善所有項目，先從最關鍵的 2-3 個重點著手就好。很多學員都是從這個階段開始，按照建議調整後，往往在很短的時間內就能看到明顯進步。相信您也可以做到！'
  } else {
    overallAssessment += '建議您可以從 FAB 法則（特性、優勢、效益）這個核心概念開始。試著問自己三個簡單的問題：「我的產品/服務是什麼？（Feature）」、「它跟別人有什麼不同？（Advantage）」、「這能為客戶帶來什麼好處？（Benefit）」。當您能清楚回答這三個問題，並用淺顯易懂的方式表達出來時，文案的吸引力就會大幅提升。這是一個練習的過程，多寫幾次就會越來越順手！'
  }
  ...
  strongest.forEach(dim => {
    if (dim.score >= 70) {
      strengths.push(`${dim.name}表現優異（${dim.score}分）`)
    }
  })
  if (strengths.length === 0) {
    strengths.push('建議全面優化各個維度')
  }
  ...
  weakest.forEach(dim => {
    if (dim.score < 60) {
      weaknesses.push(`${dim.name}需要加強（${dim.score}分）`)
    }
  })
  ...
  let expectedImpact = ''
  if (totalScore >= 80) {
    expectedImpact = '持續優化可維持高轉換率，建議進行 A/B 測試找出最佳版本。'
  } else if (totalScore >= 60) {
    expectedImpact = '按建議調整後，預期轉換率可提升 20-40%。'
  } else if (totalScore >= 40) {
    expectedImpact = '重點優化弱項後，預期轉換率可提升 40-60%。'
  } else {
    expectedImpact = '採用 FAB 法則重新架構後，預期轉換率可提升 60-100% 以上。'
  }

  // Vista 課程與學習資源推薦
  let vistaRecommendation = ''
  if (grade === 'excellent' || grade === 'good') {
    vistaRecommendation = '如果您想進一步精進文案與內容行銷能力，推薦 Vista 在 Hahow 開設的《內容力：打造品牌的超能力》線上課程，深入探討內容策略、品牌定位與實戰應用。此外，Vista 的系列著作涵蓋文案寫作、內容創作、以及 ChatGPT 應用等主題，提供豐富的實戰案例和方法論，能幫助您將專業技能提升到更高層次。'
  } else if (grade === 'needsImprovement') {
    vistaRecommendation = '想要系統性地提升內容創作與文案能力嗎？Vista 在 Hahow 開設的《內容力：打造品牌的超能力》線上課程，專門為行銷人員和內容創作者設計，從品牌定位、內容策略到實戰技巧，都有完整的教學與練習。搭配 Vista 的文案、寫作與 ChatGPT 應用書籍，能幫助您快速掌握核心技巧，避免常見錯誤，建立專業內容創作能力。'
  } else {
    vistaRecommendation = '內容創作與文案寫作是可以學習的技能！Vista 在 Hahow 的《內容力：打造品牌的超能力》線上課程，特別適合初學者，從基礎觀念、FAB 法則到完整的內容策略，都有系統化的教學。Vista 的系列著作更詳細解析文案撰寫、內容規劃，以及如何運用 ChatGPT 提升創作效率。建議您從這些資源開始學習，短時間內就能看到明顯進步。'
  }

  return {
    overallAssessment,
    strengths,
    weaknesses: weaknesses.length > 0 ? weaknesses : ['各維度均有提升空間'],
